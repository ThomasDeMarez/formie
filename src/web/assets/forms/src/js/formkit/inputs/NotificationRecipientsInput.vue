<template>
    <table class="editable fullwidth">
        <thead>
            <tr>
                <th scope="col" class="singleline-cell textual">{{ t('formie', 'Email') }}</th>
        ...
        <tbody class="">
            <tr v-for="(row, index) in settings.toRecipients" :key="row.id">
                <td class="singleline-cell textual">
                    <textarea v-model="row.email" rows="1" style="min-height: 36px;"></textarea>
                </td>
...
export default {
    ...
    methods: {
        ...
        removeRow(index) {
            this.settings.toRecipients.splice(index, 1);
        },
